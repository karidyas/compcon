...
export class NpcTemplate {
  ...
  public get FeatureSelectionInfo(): string {
    ...
    out += `optional template feature${this.OptionalMax > 1 ? 's' : ''} ${this.OptionalPerTier ? 'per NPC Tier' : ''}`
    return out
  }
  ...
  public get ClassFeatureSelectionInfo(): string {
    ...
    out += `additional optional class feature${this.OptionalClassMax > 1 ? 's' : ''} ${this.OptionalClassPerTier ? 'per NPC Tier' : ''}`
    return out
  }
